...
namespace simploce {
    ...
    public:    
         ...
         */
        explicit cvector_t(const std::vector<V>& v) : elems_{v[0], v[1], v[2]} {}
     ...
     */
    template <typename T, typename V, int D>
    T norm_square(const cvector_t<V,D>& v)
    {
    ...
    /**
     * Returns angle between two non-zero Cartesian vectors, that is, their length must be non-zero,
     * possibly of different types. Use it as
     * <pre>
    ...
    /**
     * Returns cross product between two Cartesian vectors, possibly of different types.
     * Usage:
     * <pre>
    ...
    std::ostream& operator << (std::ostream& stream, const cvector_t<V,D>& v)
    {
        ...
        stream << std::setw(conf::REAL_WIDTH) << v.x();
        stream << conf::SPACE << std::setw(conf::REAL_WIDTH) << v.y();
        stream << conf::SPACE << std::setw(conf::REAL_WIDTH) << v.z();
        return stream;
    }

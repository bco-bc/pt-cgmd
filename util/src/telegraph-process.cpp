...
namespace simploce {
    ...
    int
    TelegraphProcess::increment(int current, const stime_t& dt, const rate_t& lambda) {
    ...
    operator << (std::ostream& stream, const std::vector<pair_t>& values) {
        ...
        for (std::size_t k = 1; k != values.size(); ++k) {
	        ...
	        stream << std::setw(conf::REAL_WIDTH) << pair_k.first;
            stream << conf::SPACE << std::setw(conf::REAL_WIDTH) << pair_k.second;
	        if ( k != values.size() - 1 ) {
	            stream << std::endl;
	        }
        }

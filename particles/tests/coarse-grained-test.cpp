...
struct Update {
    void operator () (const std::vector<p_ptr_t>& particles) const
    {
...
void test()
{
    ...
    auto bead1 = cg.addBead("argon", spec1_dp);
    bead1->position(r);
    auto bead2 = cg.addBead("test2", spec1_p);
    bead2->position(r);
    cg.doWithAll<void>([](const std::vector<p_ptr_t> &particles) {
    ...
    auto bond1 = Bond::makeBond(bead1, bead2);
    ...
    auto bond2 = Bond::makeBond(bead2, pbead);
    ...
    cg2->doWithAll<void, Update>(Update{});
    std::cout << std::endl;
    
    std::cout << std::endl;
    std::cout << "Particle model 'pm':" << std::endl;
    ParticleSystem& pm = *cg2;
    pm.doWithAll<void>([] (const std::vector<p_ptr_t> &particles) {
        std::cout << "Number of particles: " << particles.size() << std::endl;
    });
    
    pm.doWithAllFreeGroups<void>([] (const std::vector<p_ptr_t>& all,

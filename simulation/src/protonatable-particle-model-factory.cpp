...
#include "simploce/particle/particle-model.hpp"
#include <vector>

namespace simploce {

    ProtonatableParticleModelFactory::ProtonatableParticleModelFactory(const spec_catalog_ptr_t& catalog) :
            ParticleModelFactory(catalog) {
    }

    prot_cg_mod
    ProtonatableParticleModelFactory::protonatablePolarizableWater(const box_ptr_t& box,
                                                                   const density_t& densitySI,
                                                                   const temperature_t& temperature,
                                                                   std::size_t nLimit) {
        util::Logger logger("ProtonatableParticleModelFactory::protonatablePolarizableWater");

        // Polarizable water.
        CoarseGrained pWater = this->polarizableWater(box, densitySI, temperature, nLimit);
        ...
        pWater.doWithAllFreeGroups<void>([&ppWater, catalog, &logger] (std::vector<bead_ptr_t>& all,
    ...
    prot_cg_mod
    ProtonatableParticleModelFactory::formicAcid(const box_ptr_t& box,
                                                 const density_t& densitySI,

...
namespace simploce {
    ...
    /**
     * Periodic boundary conditions (PBC) in one dimension (1D) with the nearest image
     * approximation, as well as specular reflection (SR) for velocities. That is, the
     * parallel component of the particle's velocity is retained, but the normal component
     * (perpendicular to the direction) is reversed (same magnitude but opposite value),
     ...
     */
    class PBC_1D_SR: public boundary_condition_impl {
    public:

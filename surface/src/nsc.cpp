...
point_double xpunsp=nullptr;
double       del_cube;
point_int    ico_wk=nullptr, ico_pt=nullptr;
...
double safe_asin(double f) {
    ...
    if ((fabs(f) - 1.00) < DP_TOL) return (M_PI_2);
        ...
        throw std::domain_error(std::to_string(f) + ": ASIN : invalid argument");
    }
}
...
void * mycalloc(const char * filename, const int linenr,
    ...
    if (ip) return (ip);
        ...
        //ERROR("CALLOC : failed in file %s at line %d", filename, linenr); return NULL; }
        throw std::domain_error("CALLOC : failed.");
    }
}
...
void * myrealloc(const char * filename, const int linenr,
  ...
  if(ip) return(ip);
      ...
      //ERROR("REALLOC : failed in file %s at line %d", filename, linenr);return NULL; }
      throw std::domain_error("REALLOC : failed");
  }
  }
...
void divarc(double x1, double y1, double z1,
  ...
  if (dd < DP_TOL) {
      ...
      throw std::domain_error("ivarc: rotation axis of length " + std::to_string(dd));
  }

  d1 = x1*x1+y1*y1+z1*z1;
  if (d1 < 0.5) {
      //ERROR("divarc: vector 1 of sq.length %f", d1);
      throw std::domain_error("divarc: vector 1 of sq.length " + std::to_string(d1));
  }
  d2 = x2*x2+y2*y2+z2*z2;
  if (d2 < 0.5) {
      //ERROR("divarc: vector 2 of sq.length %f", d2);
      throw std::domain_error("divarc: vector 2 of sq.length " + std::to_string(d2));
  }
...
/* calculate tessalation level */
  ...
  if (n_dot < densit) {
      //ERROR("ico_dot_arc: error in formula for tessalation level (%d->%d, %d)", tess, n_dot, densit);
      throw std::domain_error("ico_dot_arc: error in formula for tessalation level: " +
    std::to_string(tess) + ", " + std::to_string(n_dot) + ", " + std::to_string(densit));
  }
...
/* calculate tessalation of icosaeder faces */
    ...
    if (n_dot != tn) {
      ...
      throw std::domain_error("ico_dot: n_dot = " + std::to_string(n_dot) + ", " +
                              "tn = " + std::to_string(tn) + ", differ.");
      }
    }		/* end of if (tess > 1) */
  return n_dot;
...
/* calculate tesselation level */
  ...
  if (n_dot < densit) {
    ...
    throw std::domain_error("ico_dot_dod: error in formula for tessalation level: " + std::to_string(tess) +
    ", " + std::to_string(n_dot) + ", " + std::to_string(densit));
    }
...
int make_unsp(int densit, int mode, int * num_dot, int cubus) {
  ...
  if (mode == UNSP_ICO_ARC)      { ndot = ico_dot_arc(densit); }
    ...
    std::domain_error("make_unsp: mode " + std::to_string(mode) + " not allowed.");
    return 1;
    }
